...
for i in range(len(list_feature)):
  for letter2State in list_state:
        ...
        pathOutput = 'OUTPUT/LSTM/'+letter2State+'/'+FEATURES+'/'
        #%% Functions
        ...
        class RecurrentGCN(torch.nn.Module):  
            def __init__(self, node_features):  
                ...
                self.recurrent = LSTM(node_features, 128, 1)  
                self.linear = torch.nn.Linear(128, 1)  

            def forward(self, x, edge_index, edge_weight, h, c):

              x= x.view(1,x.shape[0],x.shape[1])
              output, (h_0, c_0) = self.recurrent(x, (h, c))
              h_0 = F.dropout(h_0, p=0.5, training=self.training)
              h = F.relu(h_0)
              h = self.linear(h)
        ...
        for iTT in range(NTT):
            ...
            for epoch in tqdm(range( NEpochs )):   # make your loops show a smart progress meter 
                cost = 0
                #H_state = None
                H_state, C_state = torch.randn(1,HIFO_num_nodes,128),torch.randn(1,HIFO_num_nodes,128)
                for timeCount, snapshot in enumerate(train_dataset):
                    y_hat, H_state, C_state = model(snapshot.x, snapshot.edge_index, snapshot.edge_attr, H_state, C_state)
                    y_hat, H_state, C_state = torch.squeeze(y_hat, 0),torch.squeeze(H_state, 0),torch.squeeze(C_state, 0)         
                    cost = cost + torch.sqrt( torch.mean((y_hat[0].t().contiguous()-snapshot.y)**2) )
                    y_hat, H_state, C_state = torch.unsqueeze(y_hat, 0),torch.unsqueeze(H_state, 0),torch.unsqueeze(C_state, 0)         
